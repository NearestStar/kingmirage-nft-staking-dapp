...
describe("NFTStakingVault.sol", () => {
  ...
  beforeEach(async () => {
    ...
    // Deploy KingMirage NFT contract
    const NFTContract = await ethers.getContractFactory("KingMirage");
    nftContract = await NFTContract.deploy(maxSupply, mintCost, maxMintAmount);

    // Deploy KingMirage ERC20 token contract
    const TokenContract = await ethers.getContractFactory("KingMirageToken");
    tokenContract = await TokenContract.deploy();

    // Deploy NFTStakingVault contract
    const Vault = await ethers.getContractFactory("NFTStakingVault");
    stakingVault = await Vault.deploy(
      nftContract.address,
  ...
  describe("Correct Deployement", () => {
    ...
    it("NFT contract should have correct initial parameters", async () => {
      ...
      await expect(nftContract.tokenURI(1)).to.be.revertedWithCustomError(
        nftContract,
        "KingMirage__QueryForNonExistentToken"
      );
    });
  ...
  describe("Core Functions", () => {
    ...
    it("should allow user to mint NFTs", async () => {
      ...
      await nftContract
        .connect(user1)
        .mint(3, { value: getAmountInWei(totalCost) });
    ...
    it("should allow user to stake its NFTs", async () => {
      ...
      for (let i = 0; i < tokenIds.length; i++) {
        await nftContract
          .connect(user1)
          .approve(stakingVault.address, tokenIds[i]);
      }
      ...
      const userNftWallet = Array.from(
        await nftContract.tokensOfOwner(user1.address),
        (x) => Number(x)
      );
      expect(userNftWallet).to.have.members([1]);

      const userStakingWallet = Array.from(
        await stakingVault.tokensOfOwner(user1.address),
        (x) => Number(x)
      );
    ...
    it("should allow user to claim reward earned from staking", async () => {
      ...
      expect(await stakingVault.getTotalRewardEarned(user1.address)).to.equal(
        0
      );
      ...
      expect(
        Math.floor(
          getAmountFromWei(
            await stakingVault.getRewardEarnedPerNft(tokenIds[0])
          )
        )
      ).to.equal(15);
      ...
      expect(
        Math.floor(
          getAmountFromWei(
            await stakingVault.getTotalRewardEarned(user1.address)
          )
        )
      ).to.equal(240);
    ...
    it("should calculate correct daily reward based on staking period", async () => {
      ...
      expect(
        await stakingVault.getDailyReward(lessThanOneMonthPeriod)
      ).to.equal(1);
      expect(
        await stakingVault.getDailyReward(lessThanThreeMonthPeriod)
      ).to.equal(2);
      expect(
        await stakingVault.getDailyReward(lessThanSixMonthPeriod)
      ).to.equal(4);
      expect(
        await stakingVault.getDailyReward(moreThanSixMonthPeriod)
      ).to.equal(8);
    });
    ...
    it("should not allow user to mint NFT while contract is paused", async () => {
      ...
      await expect(
        nftContract.connect(user1).mint(3, { value: getAmountInWei(totalCost) })
      ).to.be.revertedWithCustomError(
        nftContract,
        "KingMirage__ContractIsPaused"
      );
    });
    ...
    it("should not allow not NFT owner to stake his nfts", async () => {
      ...
      await expect(
        stakingVault.connect(user2).stake(tokenIds)
      ).to.be.revertedWithCustomError(
        stakingVault,
        "NFTStakingVault__NotItemOwner"
      );
    });
    ...
    it("should not allow user to stake same NFT twice", async () => {
      ...
      await expect(
        ...
        stakingVault,
        "NFTStakingVault__ItemAlreadyStaked"
      );
    });
    ...
    it("should not allow not NFT owner to claim staking reward", async () => {
      ...
      await expect(
        stakingVault.connect(user2).claim(tokenIds)
      ).to.be.revertedWithCustomError(
        stakingVault,
        "NFTStakingVault__NotItemOwner"
      );
    ...
    it("should not allow not NFT owner to unstake others NFTs", async () => {
      ...
      await expect(
        stakingVault.connect(user2).unstake(tokenIds)
      ).to.be.revertedWithCustomError(
        stakingVault,
        "NFTStakingVault__NotItemOwner"
      );
    ...
    it("should not allow random users to mint KPT tokens", async () => {
      ...
      await expect(
        ...
        tokenContract,
        "KingMirageToken__OnlyControllersCanMint"
      );
    });
  });
  ...
  describe("Owner Functions", () => {
    it("only owner should be allowed to change NFT contract parametres & withdraw balance", async () => {
      await expect(
        nftContract.connect(randomUser).setCost(getAmountInWei(0.01))
      ).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(
        nftContract.connect(randomUser).setMaxMintAmountPerTx(10)
      ).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(
        nftContract.connect(randomUser).setBaseURI("ipfs://new-Nft-Uri/")
      ).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(nftContract.connect(randomUser).pause(2)).to.be.revertedWith(
    ...
    it("only owner should be able to change change ERC20 contract controller", async () => {
      await expect(
        tokenContract.connect(randomUser).setController(user1.address, true)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
